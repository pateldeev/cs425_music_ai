...
parser.add_argument('--tmp_dir',
                    type=str,
                    default='/home/dp/Desktop/cs425_music_ai/ml_model/tmp')
parser.add_argument('--cache_file', type=str, default='notes_cache')
parser.add_argument(
    '--input_files_glob',
    type=str,
    default='/home/dp/Desktop/cs425_music_ai/ml_model/input_songs/*.mid')
...
# Output Params.
parser.add_argument('--final_weights_file',
                    type=str,
...
parser.add_argument(
    '--output_file',
    type=str,
    default='/home/dp/Desktop/cs425_music_ai/ml_model/model_output.mid')
...
def generate_data_sequences(notes, pitches):
    ...
    for i in range(len(notes) - args.lstm_seq_len):
        ...
        net_in_raw.append(
            np.array([note_to_int[note] for note in seq_in], dtype=np.float))
...
def train(model, x, y, batch_size, epochs):
    ...
    for f in glob.glob(
            os.path.join(args.tmp_dir,
                         TRAINING_WEIGHTS_FN[:TRAINING_WEIGHTS_FN.find('{')])):
        os.remove(f)
...
if __name__ == '__main__':
    ...
    if args.use_cache:
        ...
        input_files = args.input_files_override if args.input_files_override \
            else glob.glob(args.input_files_glob)
    ...
    net_in_raw, net_in, net_out = generate_data_sequences(notes=notes,
                                                          pitches=pitches)
    ...
    model_prediction = predict(model=model,
                               network_in_raw=net_in_raw,
                               pitches=pitches,
                               num_notes=args.output_notes)
