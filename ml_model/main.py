...
# imports music from musdb, separates it with spleeter by instrument, then stores it for late use
def separate_music():
    """testMusic = musdb.DB(subsets="test", download=True)
    trainLoc = "input_songs/trainSong"
    testLoc = "input_songs/testSong"

    fileSuffix = 1
    for song in trainMusic:
        storageLoc = trainLoc + str(fileSuffix) + ".mid"
        song1 = 0
        print(song.audio)
        song1.export(storageLoc, format="mid")
        print(storageLoc)

    converts file to wav and stores in new location
    song = AudioSegment.from_mp3(mp3TestLoc)
    song.export(wavTestLoc, format="wav")
    wavTest = wave.open(wavTestLoc, "r")"""

    trainMusic = 2  # number of songs in train music. Currently stored used a predictable convention, will late store
                    # with full song title once we create a database
    ...
    while x < trainMusic:
        ...
        cmdInput = "spleeter separate input_songs/trainSongs/trainSong" + str(x) + ".mid -p spleeter:" + str(MAX_STEMS)\
                   + "stems -o \\Users\Puppyvolcano\PycharmProjects\MusicAIAlpha\input_songs\splitSongs"
    ...
    for song_folder in os.listdir(SPLIT_INPUT_MIDI_NAME):
        for outputSongName in os.listdir(SPLIT_INPUT_MIDI_NAME + song_folder):
            if "wav" in outputSongName:
                ...
                newFileName = SPLIT_INPUT_MIDI_NAME + song_folder + "/" + outputSongName[0:outputSongName.index(".wav")] + ".mid"
                print(outputSongName)
                #TODO: convert wav to midi
                #outputSong.export(newFileName, format="mid")
                # doesn't work currently, need to make conversion from wav to midi
...
# Model params.
LSTM_SEQ_LENGTH = 10  # should be higher, initially 100
...
if __name__ == '__main__':
    ...
    while currentStem <= MAX_STEMS:
        ...
        if TRAIN_NETWORK:
            ...
            model.save_weights(os.path.join(TMP_DIR, (FINAL_WEIGHTS_FILE + str(currentStem) + ".hdf5")))
        else:
            model.load_weights(os.path.join(TMP_DIR, (FINAL_WEIGHTS_FILE + str(currentStem) + ".hdf5")))
